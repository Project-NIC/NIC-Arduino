#!/usr/bin/env python3
"""
Generate a sample NIC-MLA v1.1 file for developing the Volkov Data GUI.
...
back. The file is **self-describing**: a SCHEMA table (field names/units/scale)
and a STATION table (1-byte index → region+number) are written into the prefix
at format time, so any reader decodes raw integers into real values with no
prior knowledge. An occasional uncompressed b"PING" record exercises the
non-schema (width-mismatch) path.
...
# Three LoRa nodes — index 1..3 → (region, number, elevation_m). The 8-byte
# identity is dl_ident(region, number); elevation is a separate signed field.
    ...
    with hal:
        ...
        for r in range(N_ROUNDS):
            for idx in range(1, len(STATIONS) + 1):   # station index 1..n
                core.append(t, idx, pack_row(idx, t))
                if r % 20 == 0:                        # occasional status ping
                    core.append(t, idx, b"PING")
            t += STEP
        core.sync()
