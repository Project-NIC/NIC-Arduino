"""Shared test helpers — path bootstrap and tiny on-disk MLA builders.

All tests import this first so that the repo root is importable (``volkov_core``,
``volkov_i18n``) regardless of how the test runner sets ``sys.path``.
"""
from __future__ import annotations

import os
import struct
import sys
import tempfile

# ── make the repo root importable ────────────────────────────────────────────
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _ROOT not in sys.path:
    sys.path.insert(0, _ROOT)

# vendored MLA reference + its host-only schema tool, and NIC-DMD
_TP = os.path.join(_ROOT, "third_party")
_MLA_DIR = os.path.join(_TP, "nic_mla")
for _p in (_MLA_DIR, os.path.join(_MLA_DIR, "tools"), os.path.join(_TP, "nic_dmd")):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from nic_mla import MlaCore, MlaPosixHAL  # noqa: E402
from mla_schema import MlaSchemaBuilder, MlaStationTable, dl_ident  # noqa: E402
from nic_dmd import DmdEncoder  # noqa: E402

REPO_ROOT = _ROOT
SAMPLE_MLA = os.path.join(_ROOT, "samples", "weather.mla")

# A known schemaless fixture: (station_index, compressed, payload). Three
# uncompressed ("raw") records — two float measurements and one byte blob —
# exercising the schemaless length-guess fallback path.
FIXTURE = [
    (1, False, struct.pack("<f", 21.5)),
    (2, False, struct.pack("<f", 1013.0)),
    (1, False, b'{"msg":"hello"}'),
]


def make_temp_mla(records=FIXTURE, t0=1_748_000_000, step=900) -> str:
    """Write a small schemaless MLA with the given records; return its path."""
    fd, path = tempfile.mkstemp(suffix=".mla")
    os.close(fd)
    hal = MlaPosixHAL.create(path, file_size=64 * 1024)
    with hal:
        core = MlaCore(hal)
        core.format(file_size=64 * 1024)
        t = t0
        for station, compressed, payload in records:
            core.append(t, station, payload, compressed=compressed)
            t += step
        core.sync()
    return path


# A self-describing fixture: two int16 sensors + a one-station table at index 1.
SCHEMA_SENSORS = [("temp", "degC", 2, -1, True), ("humidity", "pct", 2, -1, False)]
SCHEMA_STATION = (7, 100)  # index 1 → region 7, number 100
SCHEMA_STATION_NAME = "Praha-Libuš"  # human station name (UTF-8, ≤32 B; diacritic)


def make_temp_mla_schema(t0=1_748_000_000, step=900) -> str:
    """Write a schema-carrying MLA: 3 packed rows + 1 odd-width record.

    Decoded: row0 temp=23.5/humidity=60.0, row1 temp=-1.5/hum=61.2,
    row2 temp=0.0/humidity=99.9. Station index 1 → region 7, number 100.
    A trailing b"PING!" record (uncompressed, "raw") has a width that does
    not match the schema, exercising the no-decode fall-through.
    """
    sb = MlaSchemaBuilder()
    sb.log("datetime")
    for name, unit, width, exp10, signed in SCHEMA_SENSORS:
        sb.data(name, unit=unit, width=width, exp10=exp10, signed=signed)
    st = MlaStationTable()
    st.station(dl_ident(region=SCHEMA_STATION[0], number=SCHEMA_STATION[1]),
               elev_m=235, name=SCHEMA_STATION_NAME)

    rows = [(235, 600), (-15, 612), (0, 999)]  # raw int16 pairs (temp, humidity)
    fd, path = tempfile.mkstemp(suffix=".mla")
    os.close(fd)
    hal = MlaPosixHAL.create(path, file_size=64 * 1024)
    with hal:
        core = MlaCore(hal)
        core.format(file_size=64 * 1024,
                    schema_table=sb.table(), station_table=st.table())
        t = t0
        for temp, hum in rows:
            payload = struct.pack("<h", temp) + struct.pack("<H", hum)
            core.append(t, 1, payload)
            t += step
        core.append(t, 1, b"PING!")  # non-matching width (uncompressed, "raw")
        core.sync()
    return path


# Same schema, but the rows are a NIC-DMD-compressed stream (keyframe + deltas).
COMPRESSED_ROWS = [(235, 600), (236, 601), (234, 599), (240, 605), (238, 603)]


def make_temp_mla_compressed(t0=1_748_000_000, step=900) -> str:
    """Write a schema MLA whose records are NIC-DMD-compressed (one station).

    Each row is (temp raw, humidity raw); decoded temp = raw/10, humidity = raw/10.
    Exercises the reader's decompress-then-decode path.
    """
    sb = MlaSchemaBuilder()
    sb.log("datetime")
    for name, unit, width, exp10, signed in SCHEMA_SENSORS:
        sb.data(name, unit=unit, width=width, exp10=exp10, signed=signed)
    st = MlaStationTable()
    st.station(dl_ident(region=SCHEMA_STATION[0], number=SCHEMA_STATION[1]),
               elev_m=235, name=SCHEMA_STATION_NAME)

    fd, path = tempfile.mkstemp(suffix=".mla")
    os.close(fd)
    hal = MlaPosixHAL.create(path, file_size=64 * 1024)
    with hal:
        core = MlaCore(hal)
        core.format(file_size=64 * 1024,
                    schema_table=sb.table(), station_table=st.table())
        enc = DmdEncoder(4)
        since_kf = 0
        t = t0
        for temp, hum in COMPRESSED_ROWS:
            payload = struct.pack("<h", temp) + struct.pack("<H", hum)
            blob = enc.compress(payload)
            since_kf = 0 if (blob[0] & 0x07) == 0 else since_kf + 1
            core.append(t, 1, blob, compressed=True, kf_back=since_kf)
            t += step
        core.sync()
    return path
