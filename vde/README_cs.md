...
> Stav: **1.2** — postaveno nad **NIC-MLA v1.2**. Dvoupanelové procházení,
> souborové operace, prohlížeč souborů i záznamů a MLA backend, který prochází
> a zobrazuje záznamy — s omezenými úpravami self-describing tabulek (není to
> obecný editor záznamů ani čistě jen prohlížeč). MLA je záměrně *hloupý*
> kontejner, takže se backend opírá o tyto tabulky: **schema tabulka** dekóduje
> každý packed payload na reálné hodnoty + jednotky a **station tabulka** přeloží
> 1bajtový index stanice z logu na reálný region/number — obojí jde rovnou do
> CSV/SQL exportu a obojí lze upravovat na místě editorem tabulek pod `F4`.
>
...
third_party/nic_mla/     přibalená NIC-MLA (Python reference) — kanonický datový formát
third_party/nic_dmd/     přibalená NIC-DMD — dekóduje komprimované (keyframe/delta) záznamy
...
Detekce a export datalogger `.mla` (víc profilů stanic v jednom souboru): `volkov_core.datalogger` — `is_datalogger()` + `export_csv()` / `export_sqlite()`. Specifikace v NIC-MLA `DESIGN-MLA-datalogger.cs.md`.
