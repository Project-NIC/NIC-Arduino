...
Core  —  Python reference implementation  (format v1.1)
...
Design — a deliberately DUMB container:
  • LOG record 16 B: offset, timestamp, subsec, length, flags, station,
    mla_crc16 — the WHOLE record is covered by the CRC. The flags byte packs
    bit7 = compressed and bits0-6 = kf_back. `subsec` is two opaque bytes the
    container only carries — MLA gives them no meaning (see MlaLog.subsec).
...
# Library/release version (semantics + API + docs). The ON-DISK FORMAT is tracked
# separately by MLA_VERSION below and stays at 1 ("format v1.1") — the v1.2 change
# (subsec redefined as two opaque glue-owned bytes + subsec_hi/lo) did NOT touch
# the wire, so old/new files stay byte-compatible.
__version__ = "1.2"

MLA_MAGIC          = b"MLA\x00"        # 4 B — prefix identifier
MLA_DATA_MAGIC     = bytes([0xAB, 0xCD])  # 2 B — sync word of every data block
MLA_VERSION        = 1                 # ON-DISK FORMAT version (= "v1.1"); unchanged
...
# Log-record flags byte (byte [12]): high bit = compressed, low 7 bits = kf_back.
# MLA stays codec-agnostic — "compressed" only means "hand the payload to the codec
# layer (e.g. NIC-DMD)"; the kind of compression / keyframe / method lives in the
# payload's own header, never here. kf_back = records back to the owning keyframe
# (0 = this record IS a keyframe), 0..127.
MLA_FLAG_COMPRESSED = 0x80
MLA_KF_MASK         = 0x7F
...
MLA_SCHEMA_VER     = 2                 # schema table version (v2: + mantissa, D50)
...
def _datalogger_byte_len(raw: bytes, off: int) -> int:
    """Total length of the datalogger tables blob at `off` (0 if not one)."""
    p = off
    if len(raw) < p + 2 or raw[p] != _DL_LOG_VER:
        return 0
    p += 2 + MLA_SCHEMA_FIELD * raw[p + 1]                 # LOG: tag n_log + fields
    if len(raw) < p + 2 or raw[p] != _DL_PROF_VER:
        return 0
    n_prof = raw[p + 1]; p += 2                            # PROFILES
...
def _schema_byte_len(raw: bytes, off: int = MLA_SCHEMA_OFF) -> int:
    ...
    if len(raw) < off + 3:
        return 0
    tag = raw[off]
    if tag == _DL_LOG_VER:                          # datalogger (profile-ref) tables
        return _datalogger_byte_len(raw, off)
    if tag != MLA_SCHEMA_VER:                        # 0x00 / 0xFF / other → none
        return 0
...
#    [29]  region_end       4 B   uint32 LE  = end of the log region. A mirror
#                                  copy of the prefix sits in [region_end ..
#                                  file_size), so region_end = file_size − prefix
#                                  size; the log grows down from region_end.
...
#    [8]  subsec     2 B  uint16 — two opaque bytes, meaning owned by the glue
#                                  layer. MLA assigns NONE and never touches them:
#                                  "subsec" reads both as sub-SECond time and as
#                                  sub-SECtion (e.g. a rotation/section index). The
#                                  glue may use it as one 16-bit value, as two
#                                  independent bytes (subsec_hi / subsec_lo), or for
#                                  several things at once — MLA only carries it.
#    [10] length     2 B  uint16 — data length 1..65535
#    [12] flags      1 B  uint8  — bit7 = compressed, bits0-6 = kf_back (0 = keyframe)
#    [13] station    1 B  uint8  — index 1..255 into the station table (0 = none)
...
_LOG_FMT     = "<IIHHBB"  # bytes 0..13 (14 B)
_LOG_CRC_LEN = 14
...
class MlaLog:
    ...
    def to_bytes(self) -> bytes:
        ...
        flags = (MLA_FLAG_COMPRESSED if self.compressed else 0) | (self.kf_back & MLA_KF_MASK)
        body = struct.pack(_LOG_FMT,
                           self.offset, self.timestamp, self.subsec,
                           self.length, flags, self.station)
    ...
    def from_bytes(cls, raw: bytes) -> tuple[MlaLog, bool]:
        ...
        flags = f[4]
        return cls(offset=f[0], timestamp=f[1], subsec=f[2], length=f[3],
                   kf_back=flags & MLA_KF_MASK,
                   compressed=bool(flags & MLA_FLAG_COMPRESSED),
                   station=f[5]), crc_ok
    ...
    # `subsec` is two opaque bytes (offset 8..9, little-endian). MLA gives them no
    # meaning; the glue layer does. These let it read/write each byte on its own —
    # e.g. high byte = section/rotation, low byte = sub-second tick — without
    # touching the wire layout (subsec_lo is byte [8], subsec_hi is byte [9]).
    @property
    def subsec_lo(self) -> int:
        return self.subsec & 0xFF

    @subsec_lo.setter
    def subsec_lo(self, value: int) -> None:
        self.subsec = (self.subsec & 0xFF00) | (value & 0xFF)
    ...
    @subsec_hi.setter
    def subsec_hi(self, value: int) -> None:
        self.subsec = (self.subsec & 0x00FF) | ((value & 0xFF) << 8)
...
class MlaCore:
    ...
    def has_room(self, data_len: int) -> bool:
        """True if a data block of `data_len` payload bytes (plus its 16 B lock)
        still fits. Lets a rotation manager predict a rotation and force a
        keyframe up front (see MlaArchive.will_rotate)."""
        block_sz = 2 + data_len + 2  # MAGIC + DATA + CRC16
        return self._top_ptr + block_sz <= self._bot_ptr - self._rs
    ...
    def format(self,
        ...
        # Prefix resilience: reserve a MIRROR copy of the prefix at the tail of
        # the file, so a single bad sector at offset 0 cannot blind the whole
        # file (mount() falls back to it). The log region therefore ends one
        # prefix-size before EOF — recorded in region_end and used everywhere as
        # the log ceiling.
        pfx_size = self._prefix.size
        self._prefix.region_end = file_size - pfx_size
        raw = self._prefix.to_bytes()
        self._hal.write(0, raw)                          # primary prefix
        self._hal.write(self._prefix.region_end, raw)    # mirror at the tail
        self._hal.sync()
        self._top_ptr = self._prefix.data_base
        self._bot_ptr = self._prefix.region_end
        self._n_slots = 0
        self._count   = 0
    ...
    def _read_prefix(self) -> MlaPrefix:
        ...
        If the primary copy at offset 0 is unreadable (bad CRC / magic), fall
        back to the mirror copy at the tail of the file.
        """
        try:
            ...
            mirror = self._read_mirror_prefix()
            if mirror is None:
                raise
            return mirror

    def _read_mirror_prefix(self) -> MlaPrefix | None:
        """Fall back to the tail mirror prefix when the primary is unreadable."""
        fs = self._hal.size()
        # Common case: a single-sector (512 B) prefix → mirror is the last sector.
        tail = self._hal.read(fs - MLA_PREFIX_SIZE, MLA_PREFIX_SIZE)
        try:
            if MlaPrefix.parse_size(tail) == MLA_PREFIX_SIZE:
                return MlaPrefix.from_bytes(tail)
        except ValueError:
            pass
        # Extended (>512 B) prefix: derive the size from the (corrupt) primary
        # header — best effort, works when the table-size bytes survived — and
        # read the whole mirror from the tail.
        try:
            size = MlaPrefix.parse_size(self._hal.read(0, MLA_PREFIX_SIZE))
            if size > MLA_PREFIX_SIZE:
                return MlaPrefix.from_bytes(self._hal.read(fs - size, size))
        except ValueError:
            pass
        return None
    ...
    def mount(self) -> None:
        ...
        lt = self._prefix.region_end   # log ceiling — the mirror sits above it

        # Binary search: slot j (0 = oldest) sits at lt - (j+1)*rs.
        max_slots = (lt - db) // rs
        lo, hi = 0, max_slots
        while lo < hi:
            mid = (lo + hi) // 2
            if self._hal.read(lt - (mid + 1) * rs, rs) == b"\xff" * rs:
                hi = mid       # slot mid is free
            else:
                lo = mid + 1   # slot mid is used
        self._n_slots = lo
        self._bot_ptr = lt - lo * rs
    ...
    def append(self, timestamp: int, station: int, data: bytes,
               *, subsec: int = 0, compressed: bool = False, kf_back: int = 0) -> None:
        """
        Append a data record. Commit protocol: LOCK first, DATA second.

        timestamp  — Unix seconds (caller's RTC/GPS).
        subsec     — two opaque bytes (0..65535) the glue owns; MLA gives them no
                     meaning and never modifies them. Use as one 16-bit value or as
                     two independent bytes (sub-second tick, section/rotation, …).
        station    — 1-byte index (1..255) into the prefix station table; the
                     library does not interpret it.
        compressed — True if `data` is a codec (DMD) blob; MLA never looks inside.
        kf_back    — records back to the owning keyframe (0 = this record is a
                     keyframe), 0..127. Compression only, else 0.
        """
        if not self._prefix:
        ...
        lock = MlaLog(offset=self._top_ptr, timestamp=timestamp, subsec=subsec & 0xFFFF,
                      length=n, kf_back=kf_back, compressed=compressed,
    ...
    def __iter__(self) -> Iterator[tuple[MlaLog, bytes]]:
        ...
        lt = self._prefix.region_end
        rs = self._rs
        for slot in range(self._n_slots):
            rec, ok = MlaLog.from_bytes(self._hal.read(lt - (slot + 1) * rs, rs))
            if not ok:
                continue
            try:
    ...
    def recover(self) -> int:
        ...
        self._prefix = self._read_prefix()
        rs = self._prefix.log_rec_size
        lt = self._prefix.region_end   # log ceiling — scan/rebuild stay below it
        if self._prefix.flags & 0x3 < MLA_CRC_DATA:
