...
>
> Logování víc typů stanic do jednoho souboru (datalogger / repeater):
> **[`DESIGN-MLA-datalogger.cs.md`](DESIGN-MLA-datalogger.cs.md)**
...
- **Volitelná komprese.** Kontejner komprimovaná data jen **označí** (jeden bit
  `compressed` v záznamu) a sleduje `kf_back` (vzdálenost zpět k vlastnímu
  keyframu); kompresní metodu nedefinuje — který kodek / keyframe je věc hlavičky
  samotného datového bloku (např. NIC-DMD).
...
- **Log záznam (16 B), celý v CRC:** offset, timestamp, subsec (dva neprůhledné
  bajty, význam dává glue), length, flags (bit7 = compressed, bity0-6 = kf_back;
  0 = keyframe), station (1bajtový index), CRC16.
...
| `nic_mla_archive.py` | Python: rotace souborů (`MlaArchive`) + hostový dotaz (`mla_query`) |
...
- **Pole `subsec` v log záznamu jsou dva neprůhledné bajty, které vlastní glue.**
  ...
  sekce / rotace). Tvoje glue (lepidlo) vrstva může 2 bajty (0..65535) brát jako
  jeden 16bitový údaj, jako dva nezávislé bajty, nebo na víc věcí naráz — např.
  horní bajt = sekce/rotace, dolní bajt = podsekundový tik, takže MLA zvládá
  vzorkování výrazně nad 1 Hz (např. seismika). Když ho nevyužiješ, nastav 0.
