...
>
> Логирование нескольких типов станций в один файл (датологгер / ретранслятор):
> **[`DESIGN-MLA-datalogger.ru.md`](DESIGN-MLA-datalogger.ru.md)**
...
- **Опциональное сжатие.** Контейнер только **помечает** сжатые данные (один бит
  `compressed` в записи) и отслеживает `kf_back` (расстояние назад до своего
  keyframe); сам метод сжатия он не определяет — какой кодек / keyframe это дело
  заголовка самого блока данных (например, NIC-DMD).
...
- **Запись журнала (16 Б), целиком под CRC:** offset, timestamp, subsec
  (два непрозрачных байта, смысл задаёт клей), length, flags (bit7 = compressed,
  биты0-6 = kf_back; 0 = keyframe), station (1-байтовый индекс), CRC16.
...
- **Поле `subsec` в записи лога — это два непрозрачных байта, которыми владеет
  клей.** MLA не придаёт им смысла и никогда их не трогает. Имя намеренно читается
  двояко: sub-**sec**ond (подсекундное) время *и* sub-**sec**tion (например, индекс
  секции / ротации). Ваш слой-клей может брать 2 байта (0..65535) как одно
  16-битное значение, как два независимых байта или сразу для нескольких вещей —
  например, старший байт = секция/ротация, младший байт = подсекундный тик, так
  что MLA справляется с дискретизацией заметно выше 1 Гц (например, сейсмика).
  Если не используется — ставьте 0.
