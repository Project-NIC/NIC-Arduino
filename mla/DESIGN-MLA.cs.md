...
- **Editace** záznamů → samostatný projekt prohlížeče *Volkov Data Ekosystém* (NIC-VDE).
- **Komprese** → volitelná, řeší ji **samostatná metoda**; kontejner komprimovaná
  data pouze **označí** (jeden bit `compressed` + vzdálenost `kf_back`), sám
...
offset 0                                                            EOF
┌────────────┬───────────────┬──────────┬─────────────┬──────────────┐
│ PREFIX     │ DATA proud →  │ volné 0xFF│ ← LOG proud  │ PREFIX zrcadlo│
│ 1–255 sekt │ (roste nahoru)│           │ (roste dolů) │ (kopie)       │
└────────────┴───────────────┴──────────┴─────────────┴──────────────┘
             ▲ data_base      ▲ top_ptr  bot_ptr ▲ region_end
```

- **DATA** rostou nahoru od `data_base` (`top_ptr` = kam přijde příští blok).
- **LOG** roste dolů od `region_end` (`bot_ptr`; příští záznam jde
...
- **PREFIX** je standardně jeden 512 B sektor; roste (po celých sektorech, max
  ...
  prefixu.
- **PREFIX zrcadlo** (přidáno v1.1): bajtově shodná kopie prefixu v posledních
  bajtech souboru (velikost prefixu), takže jediný vadný sektor na offsetu 0
  neoslepí celý soubor — `mount()` se vrátí k němu. LOG proud proto končí na
  `region_end = file_size − velikost prefixu`, těsně pod zrcadlem.
  Žádný samostatný index region — na disku není žádný strom hledání.
...
[23]  keyframe_intv   1 B   interval keyframe u komprese (výchozí 0 = N/A)
[24]  enc_caps        1 B   bitmaska kódování, která tento soubor smí nést
[25]  data_base       4 B   uint32 LE  = velikost prefixu (první bajt DATA)
[29]  region_end      4 B   uint32 LE  = file_size − velikost prefixu (konec LOG
                                         proudu; následuje zrcadlová kopie prefixu)
[33]  reserved        1 B   0
...
[2 ..]       n × 10 B záznamů (index i v logu → záznam i-1), každý:
...
Lidé čísla stanic přidělují s mezerami — glue je mapuje na kompaktní 1bajtové
indexy a zpět.
...
- **Typ dat ani délka se v bloku NEukládají** — v1.1 žádný typ záznamu vůbec není
  (soubory jsou homogenní a význam dává SCHEMA, ne značka), délka je v LOG záznamu
  (`length`, §5). Tím je datový proud čistě řízený aplikací a zápisová cesta na
  ATmega je nejmenší možná.
- Oddělovací značka `MAGIC` slouží k nalezení bloků při nouzové obnově.
- **CRC16** kryje payload.
- Konec bloku: `block_end = offset + 2 (MAGIC) + length + 2 (CRC)`.

**Příznak komprese (žádný registr typů).** v1.1 záměrně **zahodil** starý bajt
`rec_type` (raw / delta / keyframe / …). LOG záznam místo něj nese jen bajt
`flags` (§5): bit 7 `compressed` znamená „payload je blob kodeku, předej ho na
čtení kompresní vrstvě", bity 0–6 `kf_back` říkají, kolik záznamů zpět leží
vlastní keyframe (0 = tento záznam **je** keyframe). Kontejner do bloku nikdy
nenahlíží.

**Který kodek leží v datovém bloku, ne v záznamu.** Formát příznak komprese
**nese**, ale metodu **nedefinuje**. KTERÝ kodek / keyframe / varianta je
zakódováno v **hlavičce samotného datového bloku** (NIC-DMD to už dělá: bajt 0
blobu). Přidat nový kodek je tedy pár řádků v glue a kontejner se nemění.
`keyframe_intv` (prefix bajt 23) je jen **nápověda** pro čtečku — kadence, kterou
zapisovatel *zamýšlí* — a je výchozí 0; nikdy není autoritativní, tím je
`kf_back == 0`. Pokud se komprese nevyplatí, blok se uloží nekomprimovaný
(`compressed = 0`) a kontejner to nese bez rozdílu.

`enc_caps` (prefix bajt 24) je vestigiální nápověda pro čtečku, výchozí 0; nikdy
není omezení — autoritativní je hlavička každého datového bloku.

---

## 5. LOG záznam (16 B)

LOG záznam žije v LOG proudu (roste dolů od `region_end`). Má pevných **16 bajtů**
a **celý je krytý CRC** — žádné pole „flags mimo CRC".

```
[0]  offset     4 B  u32   logický offset datového bloku v DATA
[4]  timestamp  4 B  u32   Unix sekundy (dodá volající: RTC/GPS)
[8]  subsec     2 B  u16   dva neprůhledné bajty (0..65535); význam dává glue, MLA žádný
[10] length     2 B  u16   délka payloadu 1..65535
[12] flags      1 B  u8    bit7 = compressed, bity0-6 = kf_back (záznamů zpět
                           k vlastnímu keyframe; 0 = tento záznam JE keyframe)
[13] station    1 B  u8    index 1..255 do tabulky stanic v prefixu (0 = žádná)
...
Pole `subsec` (přidáno v1.1) jsou dva neprůhledné bajty, které kontejner jen
nese — MLA jim nedává žádný význam. Jméno se schválně čte oběma způsoby:
sub-**sec**ond (podsekundový) čas *i* sub-**sec**tion (např. index sekce / rotace).
Význam určuje glue (lepidlo) vrstva — může pole brát jako jeden 16bitový údaj,
jako dva nezávislé bajty, nebo na víc věcí naráz (např. horní bajt = sekce/rotace,
...
4. `recover()`: hledá `MAGIC`, zkouší délky 1..65535 dokud `CRC16(payload)`
   nesedí; obnovené záznamy se označí jako nekomprimované (`compressed = 0`,
   `kf_back = 0`), protože kódování z bloku samotného obnovit nelze.
...
- **Komprese** — MLA ji jen označí (bit `compressed`; `kf_back` váže záznam
  k jeho keyframu, 0 = keyframe). Kodek, a která varianta, je samostatný a žije
  v hlavičce samotného datového bloku.
