...
/* mla_w_append(w, ts, subsec, station, data, len, compressed, kf_back);
   ...
   subsec = dva neprůhledné bajty, význam dává glue (MLA žádný) — sub-sekundový
   čas a/nebo sekce/rotace; lze brát jako jeden u16 i jako dva bajty;
...
./mlatest /tmp/mla_c_out.bin       # 36/36 PASS; zapíše soubor pro Python cross-check
python3 cross_check.py /tmp/mla_c_out.bin   # byte-exact C↔Python ověření (10/10)
```
