# NIC-MLA v2 — Návrhová specifikace formátu
...
>
> Tento dokument definuje formát v2. **Stav implementace:** Python reference
...
Zachováváme osvědčený fyzický model v1 — **dva proudy rostoucí proti sobě**
...
- **LOG** (v v1 zvaný „index“) roste dolů od EOF (`bot_ptr`; příští záznam jde
...
|---|---|---|---|
| 0 | **Hard stop** | při zaplnění `RuntimeError` (chování v1) | jednoduché |
...
| Bity | Význam |
|---|---|
| 0–1 | režim integrity CRC: `0=NONE` · `1=DATA` · `2=FULL` (beze změny z v1) |
| 2–7 | rezerva (0) |
...
Zůstává minimální jako v1 — datový blok nese **jen ryzí data**, žádný prefix,
žádný typ:
...
- **Pole pro hostové hledání:** `timestamp`, `station`, `channel`, `rec_type`,
...
channel   = dolní 2 B počtu živých záznamů
...
[11] reserved  1 B  uint8   0xFF (budoucí: channel / flags)
```
...
**Zápis je vždy bajtově přesný** — data jdou přesně tam, kam patří (chování v1).
...
Protokol se nemění v duchu v1 — **LOCK první, DATA druhé**:
...
2. **Torn data write** (LOG OK, ale datový blok se nedopsal) → chybí `MAGIC` →
   ...
   To je klíčová obnovovací cesta z v1; mění se jen offset flags (13 → 20).
...
**Úprava**
- `MlaPrefix` — nová pole, `version=2`, `index_rec_size` → `log_rec_size=24`;
...
- **`station` / `channel`** — pokud meteostanice používá jen pár kanálů, lze
